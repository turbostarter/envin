...
<Callout type="info">
  If you **only** use the prefixed name everywhere (including server), you can
  model a single variable under **`client`** and omit an unprefixed
  `GOOGLE_CLIENT_ID`. Pick one approach per app and keep names consistent with
  how your framework exposes env vars.
</Callout>
...
For reusing the same `shared` (and other) blocks across packages in a monorepo, see [Share variables](/docs/share-variables).
