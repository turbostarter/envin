...
Your env object may extend other presets by using the `extends` property. This can be used to include system environment variables for your deployment provider, or if you have a monorepo with multiple packages that share some environment variables. For a focused guide (conventions, package layout, examples), see [Share variables](/docs/share-variables).
...
- `cloudflarePages` - [Cloudflare Pages](https://developers.cloudflare.com/pages/) system environment variables. [See full list](https://developers.cloudflare.com/pages/configuration/build-configuration/#environment-variables).
- `cloudflareWorkers` - [Cloudflare Workers Builds](https://developers.cloudflare.com/workers/ci-cd/builds/) system environment variables. [See full list](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/#environment-variables).
- `cloudflareContainers` - [Cloudflare Containers](https://developers.cloudflare.com/containers/) runtime environment variables. [See full list](https://developers.cloudflare.com/containers/configuration/environment-variables/).
...
Presets can themselves use `extends` and be nested to any depth. Types and runtime validation will merge recursively.
...
const storage = {
  id: "storage",
  server: { STORAGE_URL: z.string().url() },
} as const;
...
const env = defineEnv({
  server: { APP_SECRET: z.string().min(1) },
  extends: [api],
});
